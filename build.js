...
// the only hand-written C++ is the runtime they are compiled against (`cppsrc/rtl`:
// the headers, plus the one translation unit `cppsrc/rtl/native.cpp`) and the
// published bootstrap. So this script is a two-step build:
//
...
//   2. compile that file together with `cppsrc/rtl/native.cpp` into `simse.exe`.
//
...
//                     --define SIMSE_NO_PACK4. It reaches *both* the amalgamation and
//                     the natives, which are compiled together, so the RTL's layout
//                     choices can never disagree with the compiler's.
...
async function main() {
  ...
  if (opts.gen) {
    ...
    if (!existsSync(compiler)) {
      ...
      console.log(`build:            ${BOOTSTRAP} + ${NATIVE} -> ${path.relative(REPO, compiler)}`);
  ...
  // --- step 2: compile the amalgamation and the runtime ----------------------
  console.log(`build: compiling ${path.relative(REPO, cpp)} + ${NATIVE}`);
