...
struct and its fields - its method bodies are the `filestream` section of `_res.md` and
only the constructor-like `open` was ever in `fileio`) and the type core (`types.hpp`,
`containers.hpp`,
