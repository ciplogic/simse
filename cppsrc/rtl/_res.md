...
section per header it came from, plus `fileio`, whose text was the runtime's one hand-written
translation unit (`cppsrc/rtl/native.cpp`). Three sections carry the `emit` marker (its value
is `always`), so the compiler emits them for every program - `strtable`, the string table's
decoder, `timeops`, the clocks, and `fileio`, the platform's file/IO operations, because a
program may name any of those symbols itself with a `native(...)` declaration. Three are
